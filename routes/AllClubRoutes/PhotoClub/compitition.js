...
const requireLogin = require("../../../middleWares/requireLoginDirectorPhoto");
...
const PHOTOCOMPITITION = mongoose.model("PHOTOCOMPITITION");
...
router.post("/photocreate-compitition", async (req, res) => {
  ...
  try {
    const event = new PHOTOCOMPITITION({
      title,
      desc,
      pic,
      postedBy: req.user,
...
router.get("/photoallCompitition", (req, res) => {
  PHOTOCOMPITITION.find().then((events) => {
    res.json(events);
  });
});
...
router.get("/photogetCompitition/:compititionid", async (req, res) => {
  try {
    const competition = await PHOTOCOMPITITION.findOne({ _id: req.params.compititionid })
...
router.delete("/photocompetition/:id", async (req, res) => {
  try {
    const { id } = req.params;

    const deletedEvent = await PHOTOCOMPITITION.findByIdAndDelete(id);
...
router.put("/photoactivity/set-live/:activityID", async (req, res) => {
  try {
    ...
    // ✅ Update the field
    const updated = await PHOTOCOMPITITION.findByIdAndUpdate(
      activityId,
      { isLive },
      { new: true }
    );
...
router.post("/photoregister-compitition/:activityId", requireLogin, async (req, res) => {
  const userId = req.user._id;
  const { activityId } = req.params;

  try {
    const activity = await PHOTOCOMPITITION.findById(activityId);
...
router.post("/photounregister-compitition/:activityId", requireLogin, async (req, res) => {
  const userId = req.user._id;
  const { activityId } = req.params;
...
router.post("/photoupload-photo-compitition/:eventId", requireLogin, async (req, res) => {
  try {
    const { pic } = req.body;
    const userId = req.user._id.toString();
    const eventId = req.params.eventId;

    const event = await PHOTOCOMPITITION.findById(eventId);
...
router.get("/photohas-uploaded-compitition/:eventId", requireLogin, async (req, res) => {
  const userId = req.user._id.toString();
  const eventId = req.params.eventId;
...
router.get("/photoevent-participants-compi/:eventId", async (req, res) => {
  try {
    const { eventId } = req.params;

    // 1. Find the competition by ID and select required fields
    const event = await PHOTOCOMPITITION.findById(eventId).select("Registrations uploads");
...
router.get("/photoallJudges", async (req, res) => {
  try {
    const judges = await PHOTOJUDGE.find();
...
router.post("/photoassignJudge", async (req, res) => {
...
router.get("/photocompetition/:id/judges", async (req, res) => {
  const { id } = req.params;

  try {
    const competition = await PHOTOCOMPITITION.findById(id).populate("judges", "name email clubName");
...
router.post("/photoremoveJudge", async (req, res) => {
  const { id, judgeId } = req.body;
  ...
  try {
    const updatedCompetition = await PHOTOCOMPITITION.findByIdAndUpdate(
      id,
      { $pull: { judges: judgeId } },
      { new: true }
...
router.get("/photocompetitions/judge/:judgeId", async (req, res) => {
  const { judgeId } = req.params;

  try {
    const competitions = await PHOTOCOMPITITION.find({
      isLive: true,
...
router.patch("/photoassign-mark", async (req, res) => {
  ...
  try {
    const updated = await PHOTOCOMPITITION.updateOne(
...
router.get("/:competitionId/photoupload/:uploadId/total-score", async (req, res) => {
  ...
  try {
    const competition = await PHOTOCOMPITITION.findById(competitionId);
...
router.get("/:competitionId/photouploads/total-scores", async (req, res) => {
...
router.get("/:competitionId/photoresults", async (req, res) => {
  const { competitionId } = req.params;
  ...
  try {
    const competition = await PHOTOCOMPITITION.findById(competitionId).populate("uploads.uploadedBy", "name email");
...
router.put("/:competitionId/photogenerate-result", async (req, res) => {
  const { competitionId } = req.params;
  ...
  try {
    const updatedComp = await PHOTOCOMPITITION.findByIdAndUpdate(
      competitionId,
      { $set: { resultLive: true } },
      { new: true }
    );
...
router.get("/photocompetitions/:id", async (req, res) => {
  ...
  try {
    const competition = await PHOTOCOMPITITION.findById(id)
